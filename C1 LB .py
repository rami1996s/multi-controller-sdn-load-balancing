# controller1_LB.py (Modified for 3-controller architecture)
...
class LoadBalancingController(app_manager.RyuApp):
    ...
    def _monitor(self):
        ...
        log_file = "/tmp/kpi_log_c1_LB.csv"
        ...
        while True:
            ...
            if not valid_peers:
                self.logger.error("[C1] No valid peers available")
                hub.sleep(10)
                continue
            ...
            # === Hybrid Policy ===
            if score > min_peer_score and score_diff > 7 and len(self.datapaths) > 1:
                if score_diff > 14:
                    # migrate the heaviest switch
                    switch_to_move = max(self.packet_counts.items(), key=lambda x: x[1] if x[0] in self.datapaths else -1)[0]
                    self.logger.warning(f"[C1] Score diff > 14 → migrating HEAVIEST s{switch_to_move} to {min_peer_name}")
                else:
                    # migrate the lightest switch
                    switch_to_move = min(self.packet_counts.items(), key=lambda x: x[1] if x[0] in self.datapaths else 1e9)[0]
                    self.logger.warning(f"[C1] Score diff 7–14 → migrating LIGHTEST s{switch_to_move} to {min_peer_name}")
                    
                # Check per-switch cooldown
                now = time.time()
                last_time = self.last_migration_time.get(switch_to_move, 0)
                if now - last_time < 8:
                    self.logger.info(f"[C1] Skipping migration of s{switch_to_move} (cooldown active, migrated {now - last_time:.1f}s ago)")
                    hub.sleep(3)
                    continue    

                # Determine target port based on peer name
                target_ports = {"c2": 6634, "c3": 6635}
                target_port = target_ports.get(min_peer_name)
                
                if target_port:
                    success = self._migrate_switch(switch_to_move, target_port)
                    if success:
                        self.last_migration_time[switch_to_move] = time.time()   # record migration time
                        self.logger.info(f"[C1] Successfully migrated s{switch_to_move} to {min_peer_name}")
                        hub.sleep(3)
                    else:
                        self.logger.error(f"[C1] Migration of s{switch_to_move} failed, keeping ownership")
                else:
                    self.logger.error(f"[C1] Unknown target peer: {min_peer_name}")
                    
            elif score > min_peer_score and score_diff > 7 and len(self.datapaths) == 1:
                self.logger.info(f"[C1] Score difference {score_diff:.2f} observed , but only 1 switch → migration skipped")
    ...
    def _migrate_switch(self, dpid, target_port):
        ...
        if datapath:
            ...
            datapath.send_msg(mod)
            self.logger.info("[C1] Installed temporary fail-safe flow on %s", dpid_str)
        ...
        with self.lock:
            ...
            self.prev_packet_counts.pop(dpid, None)
            self.owned_switches = list(self.datapaths.keys())
            
        try:
            subprocess.run(cmd.split(), check=True)
            self.logger.info("[C1] Migrated %s to controller on port %d", dpid_str, target_port)

            from_ctrl = "c1"
            to_ctrl = "c2" if target_port == 6634 else "c3" if target_port == 6635 else "unknown"
            log_migration(dpid_str, from_ctrl, to_ctrl)
            return True
            
        except subprocess.CalledProcessError as e:
            self.logger.error("[C1] ovs-vsctl failed: %s\nOutput: %s", e, e.output if hasattr(e, 'output') else "N/A")
            return False
