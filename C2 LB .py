# controller2_LB.py (Modified for 3-controller architecture)
...
def report_load():
    with controller_instance.lock:
        ...
        # cumulative packet_in count (like C1)
...
class MemoryOptimizedController(app_manager.RyuApp):
    OFP_VERSIONS = [ofproto_v1_3.OFP_VERSION]

    def __init__(self, *args, **kwargs):
        super(MemoryOptimizedController, self).__init__(*args, **kwargs)
        ...
        # Modified: Multiple peer URLs for C1 and C3
        self.peer_urls = {
        ...
        Thread(target=lambda: flask_app.run(port=8081)).start()
    ...
    def packet_in_handler(self, ev):
        ...
        if eth.ethertype == ether_types.ETH_TYPE_IPV6:
           return
        # self.logger.info(f"[C2] Packet_in from dpid={dpid}, in_port={in_port}, ethertype={eth.ethertype}")
        if eth.ethertype == ether_types.ETH_TYPE_ARP:
          # self.logger.info(f"[C2] Handling ARP")
          ...
          return
            ...
            if ip_pkt is None:
                ...
                # self.logger.info(f"[C2] Invalid IPv4 packet, dropping")
                return
         
            src_ip = ip_pkt.src
            dst_ip = ip_pkt.dst
            # self.logger.info(f"[C2] Handling IP packet from {src_ip} to {dst_ip}")
            ...
            if icmp_pkt:
                # self.logger.info(f"[C2] ICMP packet detected from {src_ip} to {dst_ip}")
            ...
            # self.logger.info(f"[C2] Packet_out sent on port {out_port}")
            return 
        else:
            # self.logger.info(f"[C2] Unknown ethertype: {eth.ethertype}, dropping packet")
            return
    ...
    def switch_features_handler(self, ev):
        ...
        self.logger.info("[C2] Register switch %s", dpid_str)
    ...
    def _handle_arp(self, datapath, in_port, pkt):
        ...
        # self.logger.info("[C2] Handling ARP request %s  %s", arp_pkt.src_ip, arp_pkt.dst_ip)
    ...
    def _monitor(self):
        ...
        log_file = "/tmp/kpi_log_c2_LB.csv"
        ...
        while True:
            ...
            self.logger.info(f"[C2] CPU: {self.current_cpu:.1f}% | Memory: {self.current_mem:.1f}% | pkt_in: {total_delta} | Score: {score:.2f} | Switches: {switch_list}")

            for dpid in self.owned_switches:
                delta = deltas.get(dpid, 0)
                self.logger.info(f"[C2]  - s{dpid}: packet_in delta = {delta}")
            ...
            # === Get peer metrics from both C1 and C3 ===
            peer_scores = {}
            for peer_name, peer_url in self.peer_urls.items():
                try:
                    ...
                    self.logger.error("[C2] Error contacting %s: %s", peer_name, e)
            ...
            if not peer_scores or all(s == 0 for s in peer_scores.values()):
                self.logger.info("[C2] Peer scores not ready, skipping migration decision.")
                hub.sleep(10)
                continue
            ...
            if uptime < self.min_startup_delay:
                self.logger.info("[C2] Still in startup period (%.1fs), skipping migration", uptime)
                hub.sleep(10)
                continue 
            ...
            if not valid_peers:
                self.logger.error("[C2] No valid peers available")
                hub.sleep(10)
                continue
            ...
            self.logger.info("[C2] Score difference with %s: %.2f (Local: %.2f, Peer: %.2f)", 
            ...
            if score > min_peer_score and score_diff > 7 and len(self.owned_switches) > 1:
                if score_diff > 14 :
                    ...
                    self.logger.warning(f"[C2] Score diff > 14 → migrating HEAVIEST s{dpid} to {min_peer_name}")
                else:
                    # migrate lightest
                    dpid = min(self.packet_counts.items(), key=lambda x: x[1] if x[0] in self.owned_switches else 1e9)[0]
                    self.logger.warning(f"[C2] Score diff 7–14 → migrating LIGHTEST s{dpid} to {min_peer_name}")
                    
                # Check per-switch cooldown
                now = time.time()
                last_time = self.last_migration_time.get(dpid, 0)
                if now - last_time < 8:
                    self.logger.info(f"[C2] Skipping migration of s{dpid} (cooldown active, migrated {now - last_time:.1f}s ago)")
                    hub.sleep(3)
                    continue
                    
                # Determine target port based on peer name
                target_ports = {"c1": 6633, "c3": 6635}
                ...
                if target_port:
                    ...
                    if success:
                        ...
                        self.logger.info(f"[C2] Successfully migrated s{dpid} to {min_peer_name}")
                        hub.sleep(3)
                    else:
                        self.logger.error(f"[C2] Migration of s{dpid} failed, keeping ownership")
                else:
                    self.logger.error(f"[C2] Unknown target peer: {min_peer_name}")
                    
            elif score > min_peer_score and score_diff > 7 and len(self.owned_switches) == 1:
                self.logger.info(f"[C2] Score difference {score_diff:.2f} observed , but only 1 switch → migration skipped")
    ...
    def _migrate_switch(self, dpid, target_port):
        ...
        if datapath:
            ...
            datapath.send_msg(mod)
            self.logger.info("[C2] Installed temporary fail-safe flow on %s", dpid_str)
        ...
        try:
            ...
            self.logger.info("[C2] Migrated %s to controller on port %d", dpid_str, target_port)

            from_ctrl = "c2"
            to_ctrl = "c1" if target_port == 6633 else "c3" if target_port == 6635 else "unknown"
            log_migration(dpid_str, from_ctrl, to_ctrl)
            return True   #  explicit success
            
        except subprocess.CalledProcessError as e:
            self.logger.error("[C2] ovs-vsctl failed: %s\nOutput: %s", e, e.output if hasattr(e, 'output') else "N/A")
